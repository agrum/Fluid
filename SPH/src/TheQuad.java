...
public class TheQuad {
	...
	private void setupQuad() {
		...
		v0.setXYZ(-0.5f, 0.5f, 0); v0.setRGB(1, 0, 0); v0.setST(0, 0); //TL red
		VertexData v1 = new VertexData(); 
		v1.setXYZ(-0.5f, -0.5f, 0); v1.setRGB(0, 1, 0); v1.setST(0, 1); //BL green
		VertexData v2 = new VertexData(); 
		v2.setXYZ(0.5f, -0.5f, 0); v2.setRGB(0, 0, 1); v2.setST(1, 1); //BR blue
		VertexData v3 = new VertexData(); 
		v3.setXYZ(0.5f, 0.5f, 0); v3.setRGB(1, 1, 1); v3.setST(1, 0); //TR white
	...
	private void setupShaders() {	
		...
		m_program.addShaderFromFile(ShaderProgram.ShaderType.Vertex, "resource/classic.vert");
		m_program.addShaderFromFile(ShaderProgram.ShaderType.Fragment, "resource/classic.frag");

		// Position information will be attribute 0
		GL20.glBindAttribLocation(m_program.id(), 0, "in_Position");
		// Color information will be attribute 1
		GL20.glBindAttribLocation(m_program.id(), 1, "in_Color");
		// Textute information will be attribute 2
		GL20.glBindAttribLocation(m_program.id(), 2, "in_TextureCoord");

		m_program.link();

		// Get matrices uniform locations
		projectionMatrixLocation = GL20.glGetUniformLocation(m_program.id(),"projectionMatrix");
		viewMatrixLocation = GL20.glGetUniformLocation(m_program.id(), "viewMatrix");
		modelMatrixLocation = GL20.glGetUniformLocation(m_program.id(), "modelMatrix");
	...
	private void renderCycle() {
		...
		m_inc += 0.001;
		m_eye.lookAt(
				new Vector3f((float) Math.sin(m_inc), 0, (float) -Math.cos(m_inc)),
				new Vector3f(0, 0, 0),
				new Vector3f(0, 1, 0));
